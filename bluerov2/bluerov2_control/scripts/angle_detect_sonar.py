...
class sonar_detect:
  ...
  # -------   CALLBACK   -------
    def sonar_image_callback(self, msg):
        ...
        cv2.imshow('immagine convertita',sonar_img_cartesian)
        cv2.waitKey(1)        
        ...
        cv2.imshow('immagine con box',sonar_img_flipped)
        cv2.waitKey(1)
        ...
        alpha = 3
        sonar_polar = cv2.ellipse(sonar_polar,origin,axes,-90,-self.fov-alpha,self.fov+alpha,white,1)
        sonar_polar = cv2.ellipse(sonar_polar,origin,axes_2,-90,-self.fov-alpha,self.fov+alpha,white,1)
        sonar_polar = cv2.ellipse(sonar_polar,origin,axes_3,-90,-self.fov-alpha,self.fov+alpha,white,1)
        sonar_polar = cv2.ellipse(sonar_polar,origin,axes_4,-90,-self.fov-alpha,self.fov+alpha,white,1)
        ...
        cv2.line(sonar_polar, origin, left_corner, white,1)
        cv2.line(sonar_polar, origin, right_corner, white,1)
        ...
        for i in range (0,3):
            ...
            cv2.line(sonar_polar, origin, corner, white,1)
        ...
        cv2.putText(sonar_polar,text,right_corner,cv2.FONT_HERSHEY_SIMPLEX,0.5,white,2)
        cv2.putText(sonar_polar,text_2,corner_2,cv2.FONT_HERSHEY_SIMPLEX,0.5,white,2)
        cv2.putText(sonar_polar,text_3,corner_3,cv2.FONT_HERSHEY_SIMPLEX,0.5,white,2)
        cv2.putText(sonar_polar,text_4,corner_4,cv2.FONT_HERSHEY_SIMPLEX,0.5,white,2)
        cv2.putText(sonar_polar,text_5,corner_5,cv2.FONT_HERSHEY_SIMPLEX,0.5,white,2)
        #########################################################################
        ...
        #img_gray = cv2.cvtColor( sonar_polar,cv2.COLOR_RGB2GRAY )
        #convert to float
        #img_gray =  np.float32(sonar_polar)   
        #img_gray /= 255
        ...
        cv2.imshow("Sonar Image",sonar_polar)
        cv2.waitKey(1)
